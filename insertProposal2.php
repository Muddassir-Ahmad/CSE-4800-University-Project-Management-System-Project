...
$duplicate_id = mysqli_query($conn, "SELECT * FROM `proposal` WHERE s1_id='$s1_id' AND s2_id='$s2_id' AND project_code='CSE-3300'");
$duplicate_id1 = mysqli_query($conn, "SELECT * FROM `proposal` WHERE s1_id='$s1_id' AND s2_id='$s2_id' AND project_code='CSE-4800'");
...
if (mysqli_num_rows($duplicate_id) > 0) {
    ...
    $insert_query = "INSERT INTO `proposal`(`p_name`, `p_supervisor`,`project_code`, `s1_name`, `s1_id`,`s2_name`,`s2_id`,`proposal`) VALUES ('$p_name','$p_supervisor','p_code','$s1_name','$s1_id','$s2_name','$s2_id','$image_des')";
